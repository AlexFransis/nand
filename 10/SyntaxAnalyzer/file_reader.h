...
private:
        ...
        std::vector<std::filesystem::path>	scan_files(const std::string &input_type, const std::filesystem::path &input_path,
...
public:
        FileReader();
        std::vector<io_paths> 			get_io_paths(const std::filesystem::path &input_path, const std::string &input_type);
};
